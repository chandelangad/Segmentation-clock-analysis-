...
for fname in image_files:
    ...
    blurred = gaussian_filter(resized.astype(np.float64), sigma=5)
    ...
    # Compute boundary: last white pixel per row (right edge of white region),
    # value = distance from left side
    bin01 = (binary == 255).astype(np.uint8)
    boundary_x = np.full(h, np.nan)
    for y in range(h):
        white_idx = np.where(bin01[y, :] == 1)[0]
        if len(white_idx) > 0:
            boundary_x[y] = white_idx[-1]
